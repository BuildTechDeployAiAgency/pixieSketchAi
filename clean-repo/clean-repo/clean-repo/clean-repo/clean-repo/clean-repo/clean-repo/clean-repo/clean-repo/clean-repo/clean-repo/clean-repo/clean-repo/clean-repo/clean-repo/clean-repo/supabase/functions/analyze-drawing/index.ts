...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Received request:', { prompt: prompt?.substring(0, 100), imageUrl });

    // Validate inputs
    if (!prompt || !imageUrl) {
      console.error('Missing required fields:', { hasPrompt: !!prompt, hasImageUrl: !!imageUrl });
      return new Response(
        JSON.stringify({ 
    ...
    if (imageUrl.startsWith('blob:')) {
      ...
      return new Response(
        JSON.stringify({ 
          error: 'Invalid image format. Please upload the image to a public URL first.',
    ...
    console.log('Making OpenAI API request with key:', openAIApiKey ? 'Present' : 'Missing');
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a magical AI that analyzes children\'s drawings and creates enchanting descriptions for 3D animation. Be creative, positive, and capture the child\'s imagination in your response.'
          },
          {
            role: 'user',
            content: [
