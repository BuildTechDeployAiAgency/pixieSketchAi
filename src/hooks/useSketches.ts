...
const isValidStatus = (status: string): status is 'processing' | 'completed' | 'failed' => {
...
export const useSketches = () => {
  ...
  const checkForStuckSketches = async () => {
    try {
      ...
      if (stuckSketches && stuckSketches.length > 0) {
        console.log(`⚠️ Found ${stuckSketches.length} sketches stuck in processing for >10 minutes`);
  ...
  const fetchSketches = async (retryCount = 0) => {
    try {
      console.log(`🔄 Fetching sketches (attempt ${retryCount + 1}/${maxRetries + 1})`);
      setIsLoading(true);
      setError(null);
      ...
      const { data, error } = await supabase
        .from('sketches')
        .select('id, user_id, name, original_image_url, animated_image_url, status, is_new, created_at, updated_at')
        .eq('user_id', user.user.id)
      ...
      if (error) {
        ...
        if (error.code === '57014' && retryCount < maxRetries) {
          console.log(`⏰ Query timeout, retrying in ${(retryCount + 1) * 2} seconds...`);
          setTimeout(() => {
        ...
        toast({
          ...
          description: error.code === '57014' ? "Database query timed out. Please try refreshing the page." : "Failed to load your sketches. Please try again.",
          variant: "destructive",
        });
        setIsLoading(false);
        return;
      ...
      setNewSketchCount(convertedSketches.filter(sketch => sketch.is_new).length);
  ...
  const setupRealtimeSubscription = () => {
    ...
    console.log(`📡 Setting up real-time subscription: ${channelName} for user: ${currentUserIdRef.current}`);
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          console.log('📨 Real-time update received:', payload.eventType, newRecord?.id || oldRecord?.id);
          ...
          if (payload.eventType === 'INSERT' && newRecord) {
            ...
            console.log('🔄 Updating sketch:', updatedSketch.id, `${oldRecord.status} -> ${updatedSketch.status}`);
            ...
            if (updatedSketch.status === 'completed' && oldRecord.status === 'processing') {
            ...
            } else if (updatedSketch.status === 'failed' && oldRecord.status === 'processing') {
            ...
            setSketches(prev => prev.filter(sketch => sketch.id !== deletedId));
            if (oldRecord.is_new) {
              setNewSketchCount(prev => Math.max(0, prev - 1));
            }
          }
  ...
  const updateSketchStatus = async (id: string, status: 'processing' | 'completed' | 'failed', animatedImageUrl?: string) => {
    try {
      ...
      if (status === 'completed') {
        toast({
          ...
          description: "Your sketch has been transformed into a magical animation!",
        });
      }
    } catch (error) {
      console.error('Error updating sketch:', error);
    ...
    try {
      ...
      if (sketchToDelete) {
        ...
        if (sketchToDelete.original_image_url) {
          const originalPath = sketchToDelete.original_image_url.split('/').pop();
          if (originalPath) filesToDelete.push(originalPath);
        }
        
        if (sketchToDelete.animated_image_url) {
          const animatedPath = sketchToDelete.animated_image_url.split('/').pop();
