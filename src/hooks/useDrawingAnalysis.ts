...
export const useDrawingAnalysis = () => {
  ...
  const parseErrorMessage = (error: any): string => {
    ...
    return error?.message || error?.error || "An unexpected error occurred while analyzing your drawing.";
  };
  ...
  const analyzeDrawing = async (imageUrl: string, customPrompt?: string) => {
    ...
    try {
      const prompt = customPrompt || "Analyze this children's drawing and describe how it could be transformed into a magical drawing. Focus on the characters, colors, and potential magical elements that would bring it to life.";
