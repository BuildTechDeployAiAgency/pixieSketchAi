...
export const SketchCard = ({
  ...
  return (
    <div className={`border transition-shadow relative rounded-xl bg-white ${sketch.is_new ? "border-purple-300 shadow-lg ring-2 ring-purple-200" : "border-gray-200 hover:shadow-lg"}`}>
      {sketch.is_new && (
        <div className="absolute -top-2 -right-2 z-10">
          <Badge className="bg-red-500 text-white hover:bg-red-600 text-xs">NEW</Badge>
        </div>
      )}
      <div className="p-4 pb-2">
        <div className="flex justify-between items-start mb-2">
          <div>
            ...
            <div className="text-gray-400 text-xs">{formatDate(sketch.created_at)}</div>
          </div>
      ...
      <div className="px-4 space-y-4 mb-4">
        <div>
          <p className="text-sm font-medium text-gray-600 mb-2">Original Drawing:</p>
          <div 
            className="aspect-square bg-gray-100 rounded-lg flex items-center justify-center border cursor-pointer"
            onClick={() => sketch.original_image_url && onPreview(sketch.original_image_url)}
          >
        ...
        {sketch.status === "completed" && sketch.animated_image_url && (
          <div>
            <p className="text-sm font-medium text-gray-600 mb-2">Magic Result:</p>
            <div 
        ...
        {sketch.status === "processing" && (
          <div>
            <p className="text-sm font-medium text-gray-600 mb-2">Creating Magic:</p>
        ...
        {sketch.status === "failed" && (
          <div>
            <p className="text-sm font-medium text-gray-600 mb-2">Processing Failed:</p>
        ...
        {sketch.status === "completed" ? (
          <>
            <Button 
              ...
              variant="outline" 
              className="flex-1"
              onClick={() => sketch.animated_image_url && onDownload(sketch.animated_image_url, `${sketch.name}_animated.jpg`)}
            >
