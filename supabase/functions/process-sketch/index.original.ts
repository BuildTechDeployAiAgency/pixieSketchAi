...
serve(async (req) => {
  ...
  try {
    ...
    if (typeof imageData !== "string") {
      logWithContext("error", "Invalid image data type", {
    ...
    if (imageData.length > base64SizeLimit) {
      logWithContext("error", "Image data too large", {
        size: imageData.length,
        limit: base64SizeLimit,
      });
      return new Response(
        JSON.stringify({
    ...
    if (!PRESET_PROMPTS[preset as keyof typeof PRESET_PROMPTS]) {
      logWithContext("error", "Invalid preset provided", {
        preset,
        availablePresets: Object.keys(PRESET_PROMPTS),
      });
      return new Response(
        JSON.stringify({
    ...
    if (profile.credits <= 0) {
      logWithContext("error", "Insufficient credits", {
        userId: user.id,
    ...
    try {
      const { data: budgetCheck, error: budgetError } = await supabase.rpc(
        "check_budget_limit",
        { credits_to_use: 1 },
      );
    ...
    logWithContext("debug", "Using preset prompt", {
      preset,
...
Your response should be a detailed visual description suitable for image generation, incorporating the transformation style requested.`,
    ...
    if (!visionResponse.ok) {
      ...
      logWithContext("error", "OpenAI Vision API error", {
        status: visionResponse.status,
        error: errorText,
      });
      ...
      await handleProcessingError(
        supabase,
        sketchId,
        new Error(`Vision API error: ${errorText}`),
        "vision_analysis",
      );
      return await fallbackImageGeneration(
        openAIApiKey,
        presetPrompt,
        sketchId,
        supabase,
    ...
    if (
      ...
      await handleProcessingError(
        supabase,
        sketchId,
        new Error("Invalid vision response"),
        "vision_parsing",
      );
      return await fallbackImageGeneration(
        openAIApiKey,
    ...
    logWithContext("info", "🎯 Enhanced prompt generated", {
    ...
    if (!imageGenResponse.ok) {
      ...
      logWithContext("error", "Image generation API error", {
        status: imageGenResponse.status,
        error: errorText,
      });
      await handleProcessingError(
        supabase,
        sketchId,
        new Error(`Image generation error: ${errorText}`),
        "image_generation",
      );
    ...
    if (!animatedImageUrl) {
      logWithContext(
        "error",
        "No image URL or base64 data in response",
        imageData_result,
      );
      await handleProcessingError(
        supabase,
        sketchId,
        new Error("No image generated"),
        "image_processing",
      );
    ...
    if (sketchId) {
      try {
        ...
        logWithContext(
          "error",
          "Failed to update sketch status or refund credit",
          updateError,
        );
      }
    }
...
async function fallbackImageGeneration(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logWithContext("error", "Fallback generation failed", {
    ...
    if (animatedImageUrl) {
      ...
      logWithContext(
        "info",
        "💳 Deducting credit after successful fallback generation...",
      );
      const authHeader = req.headers.get("Authorization");
      if (authHeader) {
        const {
