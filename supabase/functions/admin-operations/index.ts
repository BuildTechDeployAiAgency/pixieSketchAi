    ...
    switch (operation) {
      ...
      case "get_stats": {
        ...
        const { data: stats, error } =
          await supabaseService.rpc("admin_get_stats");
      ...
      case "get_budget_stats": {
        ...
        const { data: stats, error } = await supabaseService.rpc(
          "admin_get_budget_stats",
        );
      ...
      case "create_budget_period": {
        ...
        if (!periodName || !totalLimit || !periodStart || !periodEnd) {
          return new Response(
            JSON.stringify({
              error:
                "periodName, totalLimit, periodStart, and periodEnd are required",
            }),
            {
              status: 400,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
        ...
        const { data: result, error } = await supabaseService.rpc(
      ...
      case "update_budget_settings": {
        const { periodId, totalLimit, alertThreshold, hardLimitEnabled } =
          params;
        ...
        if (alertThreshold !== undefined)
          updateFields.alert_threshold = alertThreshold;
        if (hardLimitEnabled !== undefined)
          updateFields.hard_limit_enabled = hardLimitEnabled;
